...
def get_contours(img):
    sobel_edges = sobel(rgb2gray(img))
    contours = find_contours(sobel_edges, 0.1)
    return contours
...
with open('C:\\Users\\kazan\\Videos\\git\\music_genres\\info\\object.txt') as f:
    object_list = f.read().splitlines()

# Путь к папке с аудиофайлами
pictures_folder = 'C:\\Users\\kazan\\Videos\\git\\music_genres\\info\\Pictures'
...
for a, picture_file in enumerate(pictures_files):
    file_path = 'C:\\Users\\kazan\\Videos\\git\\music_genres\\info\\Pictures\\' + picture_file

    features.append(get_feature(file_path))
    labels.append(object_1.index(object_list[a]))
    print(picture_file, object_list[a], object_1.index(object_list[a]))
...
y = model.predict(feature.reshape(1,498))
ind = numpy.argmax(y)
print(object_1[ind], '=> Mouse')

file_path = "C:\\Users\\kazan\\Desktop\\4.jpg"
feature = get_feature(file_path)
y = model.predict(feature.reshape(1,498))
ind = numpy.argmax(y)
print(object_1[ind], '=> Snake')